...
macro_rules! unidentified_resource {
    ($this_val:ident) => {
        /// This is a bodge to handle the fact that we don't currently have support for many of the Gms2 yy-files.
        /// Eventually, we'd like to support all of them, but downstream crates need to have some basic support until then.
        /// For now, this can be used for all top level files, providing the simplest of support.
